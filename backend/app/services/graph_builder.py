"""
图谱构建服务
接口2：使用本地Neo4j图谱构建Standalone Graph
"""
...
from ..utils.local_graph.graph_api import _node_from_record  # noqa: F401 (usato da fetch_all_nodes)

from ..config import Config
from ..models.task import TaskManager, TaskStatus
from ..utils.local_graph.neo4j_conn import get_driver
from .text_processor import TextProcessor
from ..utils.locale import t, get_locale, set_locale


# ---------------------------------------------------------------------------
# Wrappers compatibili con il codice che usava zep_paging.fetch_all_nodes/edges
# ---------------------------------------------------------------------------

def fetch_all_nodes(client: LocalGraphClient, graph_id: str, **kwargs) -> list:
    return client.graph.node.get_by_graph_id(graph_id, limit=2000)


def fetch_all_edges(client: LocalGraphClient, graph_id: str, **kwargs) -> list:
    return client.graph.edge.get_by_graph_id(graph_id, limit=2000)


# Dataclass compatibile con EpisodeData di Zep
class EpisodeData:
...
class GraphBuilderService:
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        """Salva l'ontologia nel grafo locale (semplificata rispetto a Zep Cloud)."""
        # Passo l'ontologia direttamente come dict — LocalGraphClient la serializza in JSON
        self.client.graph.set_ontology(graph_id, ontology)
